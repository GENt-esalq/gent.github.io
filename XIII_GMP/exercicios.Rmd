...
> 1 - Crie um programa para testar se o número 17933 é divisível por 79. Você pode testar a divisibilidade entre dois números usando o operador `%%`, que captura o resto da divisão entre dois números.

```{r echo=FALSE, eval=FALSE}
n = 17933
if (n %% 79 == 0) {
  print("Divisível por 79")
} else {
  print("Não é divisível por 79")
}
```

**Respostas**:

["Divisível por 79"]{.spoiler}

> 2 - Crie um programa para testar se o número 17933 é primo. Para que um número seja primo, ele deve ser divisível apenas por 1 e por ele mesmo.

```{r echo=FALSE, eval=FALSE}
n = 17933
primo = T
for (i in 2:ceiling(n^0.5)) {
  if(n %% i == 0){
    primo = F
  }
}
...
> 3 - Aproveitando o código anterior diga quais números são primos entre 150 e 200.

```{r echo=FALSE, eval=FALSE}

for (i in 150:200) {
  primo = T
  for (j in 2:ceiling(n^0.5)) {
    if(i %% j == 0){
      primo = F
    }
  }
...
[151, 157, 163, 167, 173, 179, 181, 191, 193, 197, 199]{.spoiler}

> 4 - Utilize a expressão abaixo para gerar médias de produtividade, considerando 50 genótipos de soja, de forma aleatória:

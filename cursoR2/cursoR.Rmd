...
Repare que, se posicionar o parênteses de forma incorreta, o código não resultará em nenhuma mensagem de erro, pois este é um erro que chamamos de **erro lógico**, ou seja, o código roda, mas não faz o que você gostaria que ele fizesse. Esse é o tipo de erro mais difícil de consertado. Os erros que produzem uma mensagem, seja um aviso (**warning**) ou um erro (**error**) são chamados de **erros de sintaxe**. Nesses casos, o R retornará uma mensagem para te ajudar a corrigí-los. Os **warnings** não comprometem o funcionamento do código, mas chamam a atenção para algum ponto, já os **errors** precisam necessariamente ser corrigidos para que o código rode.
...
* [dados.RData](https://GENt-esalq.github.io/wait.html) 
* [dados.csv](https://GENt-esalq.github.io/wait.html)
...
Também podemos fazer uso das funções `any` e `all`. Será que algum de nós tem mais de 60 anos de idade (idade está na 9ª coluna)? Se sim, diga para essa pessoa tomar mais cuidado com o corona vírus.

```{r}
if(any(dados[,9] > 60)){
  print("Você está no grupo de risco! Fique atento/a aos sintomas. Lave bem as mãos, use alcool gel e evite multidões.")
} else{
  print("Lave bem as mãos, use alcool gel e evite multidões.")
}
```
...
for(i in var1){
  for(j in var2){
    cat("No bloco",w,"será aplicado o",i,"na planta",j, "\n")
    w <- w + 1
  }
}
...
(https://www.reddit.com/r/ProgrammerHumor/comments/5z6n8p/every_time/)
...
Como iremos usar muito este data.frame `dados`, vamos dar nomes mais convenientes para as colunas.
...
Este material foi produzido por alunos do programa de pós-graduação em Genética e Melhoramento de Plantas. Cristiane Taniguti, Fernando Correr e Kaio Olimpio ministraram o Treinamento. Também contamos com os monitores: Letícia de Castro Lara, Getúlio Caixeta, Gabriel Gesteira, Rafael Yassue, Fernando Espolador, Júlia Morosini, Guilherme Hokasa, Victor Mello, Ana Letycia Basso, Jessica Ferrarezi e Jéssica Nogueira de Souza.
